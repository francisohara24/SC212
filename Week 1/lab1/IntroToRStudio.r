...
# Dario Fanucchi - May 2012 (original)
# Jim Scott - June 2012 (edits)
# Jim Scott - June 2016 (edits)
######################################################################################
...
# - In RStudio in the files window (bottom right screen or ctrl+5 to activate), browse
...
# [ For a COMPLETE list of keyboard shortcuts type "Alt+Shift+K" ]
...
x <- 10
# Make sure the Workspace window is visible by typing "Control+8". 
# Notice that the  Workspace window now contains a variable x, with a value of 10.
# Run the following line (with control+Enter) 
...
x  
# will produce an error: object 'x' not found!
...
# in a pop-up window. If you don't want to reach for a mouse, you can switch back to the editor
...
# Look at your workspace (Ctrl+8 if it isn't showing). There are two variables i and j that are single 
# valued variables similar to those we've seen before. i has a value of 100L. The L just refers to 
# the data type associated with i: in this case it is a "long integer". 
...
# mylist contains 9 elements, each of which is a numeric data type. Data in R is numeric
...
# Clean up the entire workspace by clicking the "Clear All" button (the broom) or by typing the following code:
remove(list=ls())                    
...
# If you need still further information, you can make use of the Help option.
# type a ? before the command like in the example below
?median
...
#   large collaborative projects in R. 
# - Standard editing tricks like commenting/uncommenting code (Ctrl+Shift+c), indentation (ctrl+I), 
...
# "manipulate" package to make your RStudio plots interactive. 
